...
function EEGVR_L2_CREx_ICA_calc(dopca, isICA, isrej, isVis)


[ALLEEG, EEG, CURRENTSET, ALLCOM] = eeglab;                % Open eeglab session
[ALLEEG, EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);

%Allows you to select and load the dataset manually.
EEG = pop_loadset();
...
icalen = find(strcmp({EEG.chanlocs.labels},{'EXG1'}))-1; % Do not take into account the external electrodes.
extchans_data = EEG.data(icalen+1:length(EEG.chanlocs),:);

pathcurr = EEG.filepath;

allfiles = dir(pathcurr)
assignin('base','allfiles',allfiles)
assignin('base','pathcurr',pathcurr)
disp({allfiles(:).name})
indx = contains({allfiles(:).name},'-info.txt')
...
if isICA==1
    ...
    deflts={'1'};
    dims = [1 40];
    rejica_ui = inputdlg(prompt,dlg_title,dims,deflts);
    ica2rej = str2num(rejica_ui{1,1});
    assignin('base','rejica_ui',rejica_ui)
    assignin('base','ica2rej',ica2rej)
...
if isrej ==1   %just carry out component rejection
    ...
    EEGnew = EEG;
    EEGnew.data = [];
    EEGnew.data = cat(1,projs,extchans_data);
    EEGnew.chanlocs = []
    EEGnew.chanlocs = chaninfo;
    EEGnew.reject.gcompreject(ica2rej) = 1;
    icarej_nom = strcat(EEGnew.setname,'-icarej');
    
    
    [ALLEEG, EEG] = eeg_store(ALLEEG, EEGnew, CURRENTSET);
    EEG = eeg_checkset( EEG );
    [ALLEEG, EEG, CURRENTSET] = pop_newset(ALLEEG, EEGnew, CURRENTSET,'setname',char(icarej_nom),'gui','off'); % current set = xx;
    EEG = eeg_checkset( EEG );
