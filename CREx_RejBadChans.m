...
function CREx_RejBadChans()
    ...
    opts.resize='on';
    chanrej_ans=inputdlg(prompt,dlg_title,[1 50; 1 50],deflts, opts);
    RejAns = split(chanrej_ans{1,1}," ");   % Split the character string using spaces
    assignin('base','RejAns',RejAns);
    ...
    if ~isempty(RejAns{1,1})
        ...
        EEG.rejchans = RejAns;
        EEG.rejchan_indx = find(ismember({EEG.chanlocs.labels},RejAns));
        ...
        if ~isequal(fid,-1)
            ...
            chanindx = zeros(size(RejAns,1),1);   %for the channel indices
            for c=1:size(RejAns,1)
                ...
                if c==size(RejAns,1)
                    ...
                    clear a;
                elseif c<size(RejAns,1)
                    fprintf(fid,'%s\t',RejAns{c,1});
                    a=RejAns{c,1};
                    chanindx(c)=find(strcmp({EEG.chanlocs.labels},cellstr(a)));  %{EEG.chanlocs.labels}
                end
            end
        else
            chanindx = zeros(size(RejAns,1),1);
            
        end
        
        assignin('base','chanindx',chanindx);
